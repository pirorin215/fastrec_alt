...
bool checkFreeSpace() {
  ...
  Serial.printf("LittleFS: Total %lu bytes, Used %lu bytes, Free %lu bytes.\r\n", totalBytes, usedBytes, freeBytes);

  if (freeBytes < minFreeBytes) {
    Serial.printf("ERROR: Not enough free space on LittleFS. Required: %lu bytes, Available: %lu bytes.\r\n", minFreeBytes, freeBytes);
    return false;
  }
  return true;
}
...
void setRtcToDefaultTime() {
  ...
  Serial.println("RTC set to default time.");
}
...
void generateFilenameFromRTC(char* filenameBuffer, size_t bufferSize) {
  ...
  Serial.println("Getting time from internal RTC for initial filename...");
  if (getValidRtcTime(&timeinfo)) {
    Serial.println(&timeinfo, "Current time from RTC: %A, %B %d %Y %H:%M:%S");
    strftime(filenameBuffer, bufferSize, "/R%Y-%m-%d-%H-%M-%S.wav", &timeinfo);
    Serial.printf("Generated filename (RTC): %s\r\n", filenameBuffer);  // Debug print
  }
}
...
void initLittleFS() {
  ...
  Serial.printf("LittleFS: Total %lu bytes, Used %lu bytes, Free %lu bytes (%.2f%% used).\r\n", totalBytes, usedBytes, freeBytes, usagePercentage);
...
void deleteAllRecordings() {
  Serial.println("Deleting all recording files.");
  onboard_led(true);
  ...
  root.close();
  onboard_led(false);
  Serial.println("Finished deleting recording files.");
...
int countAudioFiles() {
  ...
  File root = LittleFS.open("/", "r");
  if (!root) {
    Serial.println("Failed to open root directory to count audio files.");
    return 0;
  }
  ...
  root.close();
  //Serial.printf("LittleFS contains %d audio files.\r\n", currentAudioFileCount);
...
void execUpload() {
  ...
  Serial.printf("Found %d WAV files to process for upload.\r\n", numFiles);
  ...
  for (int i = 0; i < numFiles; ++i) {
    ...
    Serial.printf("Processing file for upload: %s\r\n", currentFilename);

    File audioFileForSize = LittleFS.open(currentFilename, FILE_READ);
    if (!audioFileForSize) {
      Serial.printf("ERROR: Could not open file %s to check size. Skipping.\r\n", currentFilename);
      continue;
    }
    ...
    if (uploadAudioFileViaHTTP(currentFilename, HS_HOST, HS_PORT, HS_PATH, HS_USER, HS_PASS)) {
      Serial.printf("File %s uploaded successfully. Deleting from LittleFS.\r\n", currentFilename);
      if (!LittleFS.remove(currentFilename)) {
        Serial.printf("Failed to delete file %s from LittleFS.\r\n", currentFilename);
      }
    } else {
      Serial.printf("Failed to upload file %s. Stopping further uploads.\r\n", currentFilename);
  ...
  Serial.println("Finished processing WAV files for upload.");
}
